...
//        return version.retrieveValidationResult(true);
    ...
    /**
     * HTTP controller that publishes the feed version specified in the request to all external resources enabled for the
     * application. This has the side effect of updating the publishedVersionId field on the feed version or for MTC
     * setting the processing field to true, which is updated once the {@link com.conveyal.datatools.manager.jobs.FeedUpdater}
     * has confirmed that RTD has finished processing.
     */
    private static FeedVersion publishToExternalResource (Request req, Response res) {
        FeedVersion version = requestFeedVersion(req, "manage");
        // Notify any extensions of the newly published version.
